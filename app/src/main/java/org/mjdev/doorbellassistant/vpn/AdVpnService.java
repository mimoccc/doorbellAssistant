...
import org.mjdev.doorbellassistant.activity.VPNActivity;
...
public class AdVpnService extends VpnService implements Handler.Callback {
    ...
    private final NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(
            this, NotificationChannels.SERVICE_RUNNING
    ).setSmallIcon(R.mipmap.ic_launcher).setPriority(Notification.PRIORITY_MIN);
    ...
    public static int vpnStatusToTextId(int status) {
        switch (status) {
            ...
            default:
                throw new IllegalArgumentException("Invalid vpn status value (" + status + ")");
        }
    }
    ...
    public void onCreate() {
        ...
        notificationBuilder.addAction(
                        R.mipmap.ic_launcher, // todo pause
                        getString(R.string.notification_action_pause),
                        PendingIntent.getService(
                                this,
                                REQUEST_CODE_PAUSE,
                                new Intent(this, AdVpnService.class
                                ).putExtra(
                                        "COMMAND",
    ...
    private static Intent getStartIntent(Context context) {
        ...
        intent.putExtra("NOTIFICATION_INTENT",
                PendingIntent.getActivity(
                        context,
                        0,
                        new Intent(context, VPNActivity.class),
                        PendingIntent.FLAG_IMMUTABLE
                )
        );
        return intent;
